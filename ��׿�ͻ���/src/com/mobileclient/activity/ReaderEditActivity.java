...
public class ReaderEditActivity extends Activity {
	// 声明确定添加按钮
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		iv_photo.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				intent.setClass(ReaderEditActivity.this,photoListActivity.class);
			...
			public void onClick(View v) {
				try {
					...
					if(ET_readerName.getText().toString().equals("")) {
						Toast.makeText(ReaderEditActivity.this, "姓名输入不能为空!", Toast.LENGTH_LONG).show();
					...
					if(ET_sex.getText().toString().equals("")) {
						Toast.makeText(ReaderEditActivity.this, "性别输入不能为空!", Toast.LENGTH_LONG).show();
					...
					if(ET_telephone.getText().toString().equals("")) {
						Toast.makeText(ReaderEditActivity.this, "联系电话输入不能为空!", Toast.LENGTH_LONG).show();
					...
					if(ET_email.getText().toString().equals("")) {
						Toast.makeText(ReaderEditActivity.this, "联系Email输入不能为空!", Toast.LENGTH_LONG).show();
					...
					if(ET_qq.getText().toString().equals("")) {
						Toast.makeText(ReaderEditActivity.this, "联系qq输入不能为空!", Toast.LENGTH_LONG).show();
					...
					if(ET_address.getText().toString().equals("")) {
						Toast.makeText(ReaderEditActivity.this, "读者地址输入不能为空!", Toast.LENGTH_LONG).show();
					...
					if (!reader.getPhoto().startsWith("upload/")) {
						...
						ReaderEditActivity.this.setTitle("正在上传图片，稍等...");
						String photo = HttpUtil.uploadFile(reader.getPhoto());
						ReaderEditActivity.this.setTitle("图片上传完毕！");
						reader.setPhoto(photo);
					} 
					/*调用业务逻辑层上传读者信息*/
					ReaderEditActivity.this.setTitle("正在更新读者信息，稍等...");
					String result = readerService.UpdateReader(reader);
					Toast.makeText(getApplicationContext(), result, 1).show(); 
					/*操作完成后返回到读者管理界面*/ 
					Intent intent = new Intent();
					intent.setClass(ReaderEditActivity.this, ReaderListActivity.class);
					startActivity(intent); 
					ReaderEditActivity.this.finish();
				} catch (Exception e) {}
			}
		});
	}
