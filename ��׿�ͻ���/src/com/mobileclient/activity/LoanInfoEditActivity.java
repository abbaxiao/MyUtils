...
public class LoanInfoEditActivity extends Activity {
	// 声明确定添加按钮
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		// 设置标题
		setTitle("手机客户端-修改借阅信息");
		// 设置当前Activity界面布局
		setContentView(R.layout.loaninfo_edit); 
		...
		btnUpdate.setOnClickListener(new OnClickListener() {
			public void onClick(View v) {
				try {
					...
					/*调用业务逻辑层上传借阅信息信息*/
					LoanInfoEditActivity.this.setTitle("正在更新借阅信息信息，稍等...");
					String result = loanInfoService.UpdateLoanInfo(loanInfo);
					Toast.makeText(getApplicationContext(), result, 1).show(); 
					/*操作完成后返回到借阅信息管理界面*/ 
					Intent intent = new Intent();
					intent.setClass(LoanInfoEditActivity.this, LoanInfoListActivity.class);
	...
	private void initViewData() {
		...
		this.TV_loadId.setText(loadId+"");
		for (int i = 0; i < bookList.size(); i++) {
			if (loanInfo.getBook().equals(bookList.get(i).getBarcode())) {
				this.spinner_book.setSelection(i);
				break;
			}
		}
		for (int i = 0; i < readerList.size(); i++) {
			if (loanInfo.getReader().equals(readerList.get(i).getReaderNo())) {
				this.spinner_reader.setSelection(i);
				break;
			}
		}
		Date borrowDate = new Date(loanInfo.getBorrowDate().getTime());
		this.dp_borrowDate.init(borrowDate.getYear() + 1900,borrowDate.getMonth(), borrowDate.getDate(), null);
		Date returnDate = new Date(loanInfo.getReturnDate().getTime());
		this.dp_returnDate.init(returnDate.getYear() + 1900,returnDate.getMonth(), returnDate.getDate(), null);
	}
	@Override
